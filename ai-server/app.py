    ...
    try:
        ...
        for obj, conf in detected_objects.items():
            if obj in BANNED_OBJECTS:
                ...
                violations.append({'type': vtype, 'severity': severity,
                    'message': f'{message} ({conf}%)', 'confidence': conf})
...
def reset_stats():
    stats.update({'total_frames':0,'total_violations':0,'no_face_count':0,
                  'multi_face_count':0,'object_violations':0,
                  'start_time': datetime.now().strftime('%H:%M:%S')})
